...
esp_get_munic() |>
  ...
  st_drop_geometry() |>
  left_join(
    esp_get_ccaa() |> select(codauto, ccaa.shortname.es),
    by = "codauto"
  ) |>
  select(
    municipio = name,
    caut = ccaa.shortname.es
  ) |>
  as_tibble() -> muni_ccaa
...
read_html(paste0(WIKIPEDIA_BASE_URL, "España")) |>
  ...
  transmute(
    ...
    audiencia = parse_number(audiencia, locale = locale(grouping_mark = " ")),
    cuota_pantalla = parse_number(audiencia_2, locale = locale(decimal_mark = ","))
  ) |>
...
concursantes[[edicion]] <- read_html(url) |>
  ...
  mutate(
    comunidad = as.factor(case_when(
      ...
      concursante == "Gisela Lladó" ~ "Cataluña",
      TRUE ~ comunidad
    ))
  ) |>
  arrange(posicion)
...
concursantes[[edicion]] <- read_html(url) |>
  ...
  mutate(
    comunidad = as.factor(case_when(
      ...
      concursante == "Miguel Ángel Silva" ~ "Baleares",
      TRUE ~ comunidad
    ))
  ) |>
  arrange(posicion)
...
concursantes[[edicion]] <- read_html(url) |>
  ...
  mutate(
    comunidad = as.factor(case_when(
      concursante == "Lorena Gómez" ~ "Cataluña",
      TRUE ~ comunidad
    ))
  ) |>
...
concursantes[[edicion]] <- read_html(url) |>
  ...
  mutate(
    comunidad = as.factor(case_when(
      concursante == "Chiara Oliver" ~ "Baleares",
